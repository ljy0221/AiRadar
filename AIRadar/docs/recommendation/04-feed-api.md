...
- **기사를 북마크하거나 북마크를 삭제하면 캐시가 즉시 삭제되어** 다음 요청부터 새로 계산됩니다. 북마크를 삭제하면 북마크가 프로파일에 더했던 가중치도 함께 되돌려집니다.
...
- **기사·논문을 조회하면 해당 캐시가 즉시 삭제되어** 방금 본 콘텐츠가 곧바로 이어지는 요청부터 피드에서 빠집니다. 이벤트는 비동기로 처리되므로 조회 직후 아주 짧은 시간에는 이전 결과가 나올 수 있습니다.
- **검색·온보딩처럼 프로파일이 바뀌지만 캐시를 삭제하지 않는 이벤트는 즉시 반영되지 않습니다.** 캐시가 만료되는 최대 30분 후에 반영됩니다.
- **새로고침해도 결과가 바뀌지 않을 수 있습니다.** 같은 캐시를 반환하기 때문입니다.
- **로그아웃 후 다시 로그인해도 캐시는 유지됩니다.** 사용자 ID 기반 캐시이기 때문입니다.

> **즉시 반영이 필요한 경우**: 검색 등으로 바뀐 프로파일을 피드에 바로 반영할 수는 없으므로, 최대 30분 후에 다시 확인하도록 안내하세요. 백엔드에서 캐시를 직접 무효화하는 API는 제공하지 않으며, 기사 북마크와 북마크 삭제(뉴스·논문 캐시 모두 삭제), 기사 조회(뉴스), 논문 조회(논문) 이벤트에서만 자동으로 삭제됩니다. 관심사(`user_interests`) 설정 변경은 피드에 영향을 주지 않습니다.
