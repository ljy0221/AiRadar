...
| 이벤트 가중치 | 조회=1, 검색=2, 좋아요=3, 북마크=5 | 사용자의 의도 강도를 수치로 반영 (배치에 정의된 값이며, 좋아요 이벤트 수집은 미구현) |
| Top-N | 20개/사용자 | 충분한 후보군 확보 |

**@Async 이벤트 처리**: 사용자 이벤트(조회·북마크·검색)는 `fire-and-forget`으로 즉시 202를 반환하고, 비동기로 Redis 프로파일과 DB에 기록. API 응답 지연 없음.
...
사용자 행동의 의도 강도를 수치로 반영했다. 체류 시간(30초) 기준 노이즈 필터는 설계했으나 현재 백엔드는 적용하지 않고, 프론트가 기사 클릭을 체류 0으로 전송한다.

| 이벤트 | 가중치 | 근거 |
| --- | --- | --- |
| 기사 조회 | 1.0 | 관심은 있으나 가벼운 신호 |
| 검색 (article_id가 있는 경우만 ALS에 반영) | 2.0 | 의도적 탐색 행동 |
| 좋아요 (이벤트 수집 미구현) | 3.0 | 명시적 긍정 반응 — 배치에는 정의되어 있음 |
